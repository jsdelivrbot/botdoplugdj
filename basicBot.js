...
(function () {
    ...
    var loadChat = function (cb) {
        ...
        $.get("https://raw.githubusercontent.com/rafaell320/botdoplugdj/master/lang/languages.json", function (json) {
    ...
    var basicBot = {
        ...
        settings: {
            ...
            rulesLink: null,
            themeLink: null,
            fbLink: "https://www.facebook.com/groups/676108085821080/",
            youtubeLink: null,
            website: "https://www.facebook.com/panelinharadioativa",
            intervalMessages: [],
            messageInterval: 5,
            songstats: false,
            commandLiteral: "!",
        ...
        commands: {
            ...
            cookieCommand: {
                ...
                rank: 'user',
                type: 'startsWith',
                cookies: ['te deu um cookie de gotas de chocolate!',
                    'te deu um delicioso cookie caseiro!',
                    'te deu um velho e seco cookie, era o ultimo do pacote, Grosso.',
                    'te deu um cookie confeitado, O que?!, sem chantilly e confeitos!? 0/10.',
                    'te deu um cookie de gotas de chocolate! espera, isso sao passas!! *nojinho*',
                    'te deu um cookie de barro importado da África',
                    'te deu um biscoito da sorte escrito: "Por que você nao está trabalhando?"',
                    'te deu um biscoito da sorte escrito: "Me beija, seu delícia."',
                    'te deu um biscoito da sorte escrito: "Arrume uma namorada!"',
                    'te deu um biscoito da sorte escrito: "Vá viver"',
                    'te deu um biscoito da sorte escrito: "Não esqueça de comer seus vegetais!"',
                    'te deu um biscoito da sorte escrito: "Você vai à academia?"',
                    'te deu um biscoito da sorte escrito: "m808 pls"',
                    'te deu um biscoito da sorte escrito: "If you move your hips, you\'ll get all the ladies."',
                    'te deu um biscoito da sorte escrito: "eu te amo"',
                    'te deu um cookie dourado, e você não pode come-lo por que é feito de ouro. Droga.',
                    'te deu um cookie de oreo com um copo de leite!',
                    'te deu um cookie de arco-íris feito com muito amor :heart:',
                    'nao te deu cookie nenhum, mas ele tem outras coisas que pode dar ( ͡° ͜ʖ ͡°).',
                    'fez para voce cookies fresquinhos, estão com um cheiro maravilhouser.'
                ],
                getCookie: function () {
		...
		baterCommand: {
                ...
                baters: [
                    ...
                    'te deu um soco no nariz',
                    'te deu um sapatada tamanho 45 na cara',
                    'te deu um chute no saco',
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.batenimim);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(basicBot.chat.nouserbater, {name: name}));
                            }
                            else if (user.username === chat.un) {
                                return API.sendChat(subChat(basicBot.chat.selfbater, {name: name}));
                            }
                            else {
                                return API.sendChat(subChat(basicBot.chat.bater, {nameto: user.username, namefrom: chat.un, bater: this.getBater()}));
                            }
                        }
                    }
                }
...
chantagearCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.mechantageia);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
                            var user = basicBot.userUtilities.lookupUserName(name);
                            if (user === false || !user.inRoom) {
                                return API.sendChat(subChat(basicBot.chat.nouserchantagear, {name: name}));
                            }
                            else if (user.username === chat.un) {
...
punirCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var space = msg.indexOf(' ');
                        if (space === -1) {
                            API.sendChat(basicBot.chat.mepune);
                            return false;
                        }
                        else {
                            var name = msg.substring(space + 2);
